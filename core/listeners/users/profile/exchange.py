...
class ExchangeListener(commands.Cog):
    ...
    @custom_id_check("exchange_money_menu")
    @callback_data_injection()
    async def exchange_money_menu_listener(
        self, 
    ...
    async def exchange_quartz_listener(
        ...
        quartz_type = self.types[callback.character_id]
        exchange_info = self.bot.conf.exchange_info['quartz_to_quartz']
        value = exchange_info[quartz_type]
        ...
        await self.bot.db.update_user_money(inter.author.id, quartz_type, -1)
        await self.bot.db.update_user_money(inter.author.id, "quartz", +int(value))
